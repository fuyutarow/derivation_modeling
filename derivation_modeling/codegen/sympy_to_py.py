...
from derivation_modeling.codegen.transforms import extract_free_variables, extract_sum
...
def convert(e, definitions=[], func_name='func', extra_args=[], index_trans={}):
    ...
    for d in definitions:
        define = py_assign_stmt(
            py_var(str(d.lhs)), expr_to_py()(d.rhs), py_assign_stmt.PY_ASSIGN_EQUAL)
...
# convert multidimensional integral as iterated integral
    ...
    def convert_integral(self, e):
        ...
        for idx, v in enumerate(var_list[:-1]):
            ...
            trap_call = py_function_call('trap' + str(idx + 1),
                                         py_arg_list(
                                             py_var(a),
                                             py_var(b),
                                             py_var('f_' + var_list[idx + 1]), n, *other_args))
        ...
        call = py_function_call('trap0',
                                py_arg_list(py_var(a), py_var(b), py_var('f_' + var_list[0]), n))
...
if __name__ == '__main__':
    ...
    x = IndexedBase('x')
    index_trans = {}
    index_trans['x'] = py_expr(py_expr.PY_OP_PLUS,
                               py_var('a'), py_expr(py_expr.PY_OP_TIMES, py_var('i'), py_var('h')))
    #e = x[i]
    #e = 1/b
    #e = a-5*b
    #e = 1+a+b
