...
def convert_one_integral(e, func_trans=[], unique_id=''):
    ...
    for idx, v in elist[1:]:
        ...
        #trap_call = c_function_call('trap'+str(idx+1),c_var(a), c_var(b), c_var(unique_id + 'f_'+str(var_list[idx-1])),n,*other_args)
        trap_call = c_function_call('trap' + str(idx + 1),
                                    c_var(a), c_var(b), c_var(func_to_call_name), n, *other_args)
    ...
    call = c_function_call('trap0',
                           c_var(a), c_var(b), c_var(unique_id + 'f_' + str(var_list[0])), n)
    #v = c_double('v')
